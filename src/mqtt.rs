...
use crate::models::{MeshWatchyError, MqttConfig, RawMqttMessage};
...
impl MqttClient {
        ...
        loop {
            match eventloop.poll().await {
                Ok(Event::Incoming(Packet::Publish(publish))) => {
                    ...
                    if let Err(e) = Self::process_mqtt_message(
                        &topic,
                        &payload,
                        &database,
                        &webhook_manager,
                        ws_broadcaster.as_deref(),
                    )
                    .await
                    {
    ...
    async fn process_mqtt_message(
        ...
        if let Err(e) = database.insert_message(&raw_message, &clean_topic).await {
            ...
            tracing::debug!(
                "Stored message ID {} from node {} (type: {})",
                raw_message.id,
                raw_message.from,
    ...
    fn parse_raw_payload(payload: &str, topic: &str) -> Result<RawMqttMessage, MeshWatchyError> {
        ...
        let sender = topic_parts.get(5).unwrap_or(&"unknown").to_string();
    ...
    pub fn infer_message_type(payload: &serde_json::Value) -> crate::models::MessageType {
        ...
        if let Some(obj) = payload.as_object() {
            ...
            if obj.contains_key("battery_level")
                || obj.contains_key("voltage")
                || obj.contains_key("air_util_tx")
            {
                tracing::debug!(
                    "Detected TELEMETRY message - contains battery/voltage/air_util data"
                );
                return MessageType::Telemetry;
            }

            // Check for node info data
            if obj.contains_key("longname")
                || obj.contains_key("shortname")
                || obj.contains_key("hardware")
            {
...
impl MqttManager {
    ...
    pub async fn start(
        ...
        client
            .start(database, webhook_manager, ws_broadcaster)
            .await?;
