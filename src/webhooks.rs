use crate::models::{MeshWatchyError, MessageType, RawMqttMessage, WebhookConfig, WebhookEvent};
use chrono::Utc;
use reqwest::Client;
...
impl WebhookManager {
        ...
        let matching_webhooks: Vec<WebhookConfig> = webhooks
            ...
            .filter(|webhook| {
                ...
                if !webhook.message_types.is_empty()
                    && !webhook.message_types.contains(&message.message_type)
                {
                    return false;
                }
    ...
    async fn send_webhook(
        ...
        // Prepare request
        let mut request_builder = client.post(&webhook.url).json(&event);
    ...
    pub async fn test_webhook(
        &self,
        webhook: &WebhookConfig,
    ) -> Result<TestResult, MeshWatchyError> {
        ...
        let mut request_builder = self.client.post(&webhook.url).json(&test_event);
