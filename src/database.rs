...
impl Database {
    ...
    pub async fn insert_message(
        ...
        match &raw_message.message_type {
            MessageType::Position => {
                if let Ok(position) =
                    serde_json::from_value::<PositionPayload>(raw_message.payload.clone())
                {
                    self.insert_position_data(&mut tx, message.id, &position)
                        .await?;
                }
            }
            MessageType::Nodeinfo => {
                if let Ok(node_info) =
                    serde_json::from_value::<NodeInfoPayload>(raw_message.payload.clone())
                {
                    self.insert_node_info(&mut tx, &node_info, raw_message.timestamp)
                        .await?;
                }
            }
            MessageType::Telemetry => {
                if let Ok(telemetry) =
                    serde_json::from_value::<TelemetryPayload>(raw_message.payload.clone())
                {
                    self.insert_telemetry_data(&mut tx, message.id, &telemetry)
                        .await?;
                }
            }
            MessageType::Text => {
                if let Ok(text_payload) =
                    serde_json::from_value::<TextMessagePayload>(raw_message.payload.clone())
                {
                    self.insert_text_message(&mut tx, message.id, &text_payload)
                        .await?;
                }
            }
            MessageType::Unknown => {
    ...
    async fn insert_node_info(
        ...
        tracing::debug!(
            "Inserted/updated node info for {}: {} ({})",
            node_info.id,
            node_info.longname,
            node_info.shortname
        );
    ...
    pub async fn get_recent_positions(
        &self,
        limit: i32,
    ) -> Result<Vec<PositionResponse>, MeshWatchyError> {
        ...
        for (node_id, timestamp, latitude, longitude, altitude, battery_level, rssi, snr) in
            positions
        {
    ...
    pub async fn get_recent_positions_with_info(
        &self,
        limit: i32,
    ) -> Result<Vec<PositionResponseDisplay>, MeshWatchyError> {
        ...
        .map_err(|e| {
            tracing::error!(
                "Database query failed in get_recent_positions_with_info: {}",
                e
            );
            e
        })?;
        ...
        {
            tracing::debug!(
                "Processing position {}: node_id={}, hardware_type={:?}",
                i,
                node_id,
                hardware_type
            );
        ...
        tracing::debug!(
            "Successfully created {} PositionResponseDisplay objects",
            result.len()
        );
        Ok(result)
    }
    ...
    pub async fn get_recent_positions_filtered(
        &self,
        limit: i32,
        ...
        let positions = query.fetch_all(&self.pool).await.map_err(|e| {
            tracing::error!(
                "Database query failed in get_recent_positions_filtered: {}",
                e
            );
            e
        })?;
    ...
    pub async fn get_network_overview(&self) -> Result<NetworkOverview, MeshWatchyError> {
        ...
        let active_nodes: i32 =
            sqlx::query_scalar("SELECT COUNT(DISTINCT node_id) FROM node_info WHERE last_seen > ?")
                .bind(day_ago)
                .fetch_one(&self.pool)
                .await?;

        // Get messages today
        let messages_today: i32 =
            sqlx::query_scalar("SELECT COUNT(*) FROM mesh_messages WHERE timestamp > ?")
                .bind(day_ago)
                .fetch_one(&self.pool)
                .await?;
        ...
        let last_message_ts: Option<i64> =
            sqlx::query_scalar("SELECT MAX(timestamp) FROM mesh_messages")
                .fetch_one(&self.pool)
                .await?;

        let last_message = last_message_ts.and_then(|ts| DateTime::<Utc>::from_timestamp(ts, 0));
    ...
    pub async fn get_nodes_display(
        &self,
    ) -> Result<Vec<crate::models::NodeInfoDisplay>, MeshWatchyError> {
    ...
    pub async fn get_recent_text_messages(
        &self,
        limit: i32,
    ) -> Result<Vec<TextMessage>, MeshWatchyError> {
        let rows = sqlx::query(
            r#"
            SELECT 
                mm.id as message_id,
        ...
        for row in rows {
            ...
            let gateway = mqtt_topic.split('/').nth(1).map(|s| s.to_string());
    ...
    pub async fn get_recent_messages(
        &self,
        limit: i32,
    ) -> Result<Vec<RecentMessage>, MeshWatchyError> {
        let rows = sqlx::query(
            r#"
            SELECT 
                id,
    ...
    pub async fn get_recent_telemetry(
        &self,
        limit: i32,
    ) -> Result<Vec<TelemetryDisplay>, MeshWatchyError> {
        let rows = sqlx::query(
            r#"
            SELECT 
                printf('!%08x', mm.from_node) as node_id,
        ...
        let mut node_data: std::collections::HashMap<String, Vec<TelemetryDisplay>> =
        ...
        for (_node_id, mut node_telemetry) in node_data {
            ...
            let battery_levels: Vec<Option<i32>> =
                node_telemetry.iter().map(|t| t.battery_level).collect();
    ...
    pub async fn get_node_telemetry_history(
        ...
        let _complete_battery_chart =
            TelemetryDisplay::battery_levels_to_time_chart(&battery_levels);
...
mod tests {
    use super::*;
    use crate::models::{
        MessageType, NodeInfoPayload, PositionPayload, RawMqttMessage, TelemetryPayload,
    };
    ...
    async fn test_insert_position_message() {
        ...
        db.insert_message(&raw_message, "msh/Zoo/2/json/ZooNet/!f9943e58")
            .await
            .unwrap();
