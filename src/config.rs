use crate::models::{AppConfig, DatabaseConfig, MessageType, MqttConfig, WebConfig, WebhookConfig, MeshWatchyError};
...
pub async fn load_config(path: &str) -> Result<AppConfig, MeshWatchyError> {
    if std::path::Path::new(path).exists() {
        ...
        let config_str = toml::to_string_pretty(&default_config)
            .map_err(|e| MeshWatchyError::Config(format!("Failed to serialize default config: {}", e)))?;
        
        fs::write(path, config_str)
            .map_err(|e| MeshWatchyError::Config(format!("Failed to write default config: {}", e)))?;
...
pub async fn load_webhook_config(path: &str) -> Result<Vec<WebhookConfig>, MeshWatchyError> {
    if std::path::Path::new(path).exists() {
        let webhook_str = fs::read_to_string(path)
            .map_err(|e| MeshWatchyError::Config(format!("Failed to read webhook config: {}", e)))?;
        ...
        let webhook_file: WebhookFile = toml::from_str(&webhook_str)
            .map_err(|e| MeshWatchyError::Config(format!("Failed to parse webhook config: {}", e)))?;
...
pub async fn save_webhook_config(path: &str, webhooks: &[WebhookConfig]) -> Result<(), MeshWatchyError> {
    ...
    let webhook_str = toml::to_string_pretty(&webhook_file)
        .map_err(|e| MeshWatchyError::Config(format!("Failed to serialize webhook config: {}", e)))?;
...
fn create_example_webhook_config() -> Vec<WebhookConfig> {
    ...
    vec![
        ...
        WebhookConfig {
            ...
            enabled: false,
            message_types: vec![MessageType::Position, MessageType::Nodeinfo, MessageType::Telemetry],
            headers: None,
            devices: vec![],
        },
    ]
...
pub fn validate_config(config: &AppConfig) -> Result<(), MeshWatchyError> {
    ...
    if config.mqtt.broker_host.is_empty() {
        return Err(MeshWatchyError::Config("MQTT broker host cannot be empty".to_string()));
    }
    
    if config.mqtt.broker_port == 0 {
        return Err(MeshWatchyError::Config("MQTT broker port must be valid".to_string()));
    }
    
    if config.mqtt.topics.is_empty() {
        return Err(MeshWatchyError::Config("At least one MQTT topic must be specified".to_string()));
    }
    
    // Validate database config
    if config.database.url.is_empty() {
        return Err(MeshWatchyError::Config("Database URL cannot be empty".to_string()));
    }
    
    if config.database.max_connections == 0 {
        return Err(MeshWatchyError::Config("Max connections must be greater than 0".to_string()));
    }
    
    // Validate web config
    if config.web.bind_port == 0 {
        return Err(MeshWatchyError::Config("Web server port must be valid".to_string()));
    }
    ...
    for webhook in &config.webhooks {
        if webhook.enabled {
            if webhook.url.is_empty() {
                return Err(MeshWatchyError::Config(format!("Webhook '{}' URL cannot be empty", webhook.name)));
            }
            
            if !webhook.url.starts_with("http://") && !webhook.url.starts_with("https://") {
                return Err(MeshWatchyError::Config(format!("Webhook '{}' URL must start with http:// or https://", webhook.name)));
            }
        }
    }
