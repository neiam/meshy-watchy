...
async fn main() -> anyhow::Result<()> {
    ...
    tracing::info!("Webhook manager initialized with {} webhooks", config.webhooks.len());
    ...
    env.add_filter("strftime", |value: &minijinja::Value, format: String| -> Result<String, minijinja::Error> {
        if let Some(dt) = value.as_str() {
            ...
            if let Ok(parsed_dt) = chrono::DateTime::parse_from_rfc3339(dt) {
                ...
                Err(minijinja::Error::new(minijinja::ErrorKind::InvalidOperation, "Could not parse datetime"))
            }
        } else {
            Err(minijinja::Error::new(minijinja::ErrorKind::InvalidOperation, "Value is not a datetime string"))
        }
    });
    
    // Add custom relative time filter for "n minutes ago" formatting
    env.add_filter("timeago", |value: &minijinja::Value| -> Result<String, minijinja::Error> {
        let timestamp = if let Some(ts) = value.as_i64() {
            ts
        } else if let Some(ts_str) = value.as_str() {
            ts_str.parse::<i64>().map_err(|_| {
                minijinja::Error::new(minijinja::ErrorKind::InvalidOperation, "Could not parse timestamp")
            })?
        } else {
    ...
    tracing::info!("Template environment initialized with {} templates", env.templates().count());
    ...
    // Start web server
    tracing::info!("Starting web server on {}:{}", config.web.bind_address, config.web.bind_port);
